  ...
  if (!NON_SLA_EVENTS.includes(event.payload.sla)) {
    ...
    if (!(await isPull.get())) {
      chrome.notifications.create('', {
        ...
        message: 'You can pull new work now',
...
async function checkTimer() {
  if (
    (isPresent(lastTimer) &&
      isPresent(lastRun) &&
      differenceInSeconds(lastRun, new Date()) > 2) ||
    !isInitialized
  ) {
...
function lowTimeNotification(sla: string) {
  chrome.notifications.create('', {
    title: 'Hurry up',
    message: sla,
    iconUrl: '/icons/icon_128.png',
...
function setBadge(event: SlaEvent) {
  if (isPresent(event.payload.sla)) {
    chrome.action
      .setBadgeText({
...
async function setNotifications(event?: SlaEvent) {
  ...
  setBadge(event);
  if (
    isPresent(event.payload.slaObject) &&
    isPresent(event.payload.slaObject.h) &&
    isPresent(event.payload.slaObject.m)
  ) {
    ...
    if (
      event.payload.slaObject.h >= 0 &&
      event.payload.slaObject.m > 10 &&
      (lastTenMinutesValue || lastFiveMinutesValue || lastMinuteValue)
    ) {
      await Promise.all([
        lastMinute.set(false),
        lastFiveMinutes.set(false),
    ...
    } else if (isPresent(event.payload.sla)) {
      if (
        !lastTenMinutesValue &&
        event.payload.slaObject.h < 1 &&
        event.payload.slaObject.m < 10
      ) {
        lowTimeNotification(event.payload.sla);
        await lastTenMinutes.set(true);
      } else if (
        !lastFiveMinutesValue &&
        event.payload.slaObject.h < 1 &&
        event.payload.slaObject.m < 5
      ) {
        lowTimeNotification(event.payload.sla);
