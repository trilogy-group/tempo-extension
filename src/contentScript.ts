...
async function getHistory(): Promise<HistoryObject | undefined> {
  const keys = Object.keys(localStorage).filter(
    (x) =>
      x.startsWith('CognitoIdentityServiceProvider.') && x.endsWith('.idToken')
  );
  if (keys.length === 0) {
...
async function loadSla() {
  ...
  if (pullWork.length > 0) {
    ...
    const slaSelector = document.querySelector(
      '#__next > div > div > div > div > div > div > div > div > div > div > div > div > div > div > div > span'
    );
    const slaText = slaSelector?.textContent ?? '';
    const slaObjectRegex =
      /^((\d{1,2})h\s*)?((\d{1,2})m\s*)?((\d{1,2})s\s*)?$/g;
